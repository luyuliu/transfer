...
def calculateDiff(BTimeString,ATimeString):
    ...
    if Bs[1]=="AM":
        ...
        additionalHour=12
    time=Bs[0].split(":")
    hours=int(time[0])+additionalHour
    minutes=int(time[1])
    seconds=int(time[2])
    BTotalSeconds=hours*3600+minutes*60+seconds

    As=ATimeString.split(" ")
    #print(As)
    if As[1]=="AM":
        additionalHour=0
    else:
...
#trips
with open(dataLocation+'\\trips.csv', newline='') as tripsFile:
        ...
        if distance>searchRadius:
            ...
            for Bj in stopsDic[BStop_id]["stop_trip_id"]:#Bj: The trip_id in a stopDic stop record
                ...
                for Ai in stopsDic[AStop_id]["stop_trip_id"]:#Ai: The trip_id in a stopDic stop record
                    ...
                    if scheduleDiff<realWalkingTime or scheduleDiff<=0:
                        continue;
                    if closestScheduleDiff>scheduleDiff:
                        closestScheduleDiff=scheduleDiff
                        closestATripId=Ai
                ...
                if closestATripId==-1:
                    continue
                line={}
                line["a_stid"]=AStop_id
                line["a_tid"]=closestATripId
                line["a_sid"]=tripsDic[closestATripId]["service_id"]
                
                line["b_stid"]=BStop_id
                line["b_tid"]=Bj
                line["b_sid"]=tripsDic[Bj]["service_id"]
                line["diff"]=closestScheduleDiff
                line["w_time"]=round(realWalkingTime,2)
                
                transferDic.append(line)
                print(line)
