...
for single_date in daterange(start_date, end_date):
    ...
    for each_transfer in db_schedule:
        ...
        if b_trip_id != "621874" and a_trip_id != "621870":
            continue
        ...
        b_real_time = -1
        b_trip_feeds = list(db_tripupdate.find({"start_date": str(today_date), "trip_id": str(
            b_trip_id), "seq": {"$elemMatch": {"stop": b_stop_id}}}))
        ...
        a_real_time = -1
        a_trip_feeds = list(db_tripupdate.find({"start_date": str(today_date), "trip_id": str(
            a_trip_id), "seq": {"$elemMatch": {"stop": a_stop_id}}}))
        ...
        for each_trip in false_trips_list:  # For each trips, find the real departure time for it, which still satisfies the requirement: the departure time is after
            ...
            stop_time_query = list(db_GTFS.stop_times.find(
                {"trip_id": i_trip_id, "stop_id": b_stop_id}))  # Because some trips do not share the exactly same shape.
            # end_time=time.time()
        ...
        for each_trip in false_trips_list:
            ...
            if len(alternative_b_trip) == 0:
                print("i_trip_id", i_trip_id, "|", "each_schedule", convertSeconds(
                    each_trip["b_time"], single_date), "each_b_time", each_b_time, "id", "|", each_trip["sequence_id"])
                continue
            for b_stop_time in alternative_b_trip[len(alternative_b_trip) - 1]["seq"]: # From the last records containing the b_stop (which is the most accurate record)
                if b_stop_time["stop"] == b_stop_id:
        ...
        print("schd:", real_transfer["b_real_time"],
              real_transfer["b_trip_id"], real_transfer["b_time"])

        if real_transfer["status"] == 0 and (b_alt_time != b_real_time):
