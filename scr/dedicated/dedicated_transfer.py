...
def paralleling_transfers(single_date, dedicated_route_id):
    ...
    db_dedicated_collection = client.cota_dedicated["D_" + today_date + "_" + str(dedicated_route_id)]
...
if __name__ == '__main__':
    ...
    for dedicated_route_id in range(1, 11):
        if dedicated_route_id == 2:
            continue
        date_range = list(daterange(start_date, end_date))
        cores = multiprocessing.cpu_count()
        pool = multiprocessing.Pool(processes=30)
        output = []
        output = pool.starmap(paralleling_transfers, zip(date_range,[dedicated_route_id]*len(date_range)))
        pool.close()
        pool.join()
