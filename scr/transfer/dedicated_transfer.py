    ...
    for single_result in db_validated_transfer:
        ...
        if single_result["status"]<3 or single_result["status"]==6:
            ...
            if single_result["a_ro"]==dedicated_route_id or single_result["a_ro"]==-dedicated_route_id:
                ...
                real_b_seq=list(db_seq.find({"service_id":str(service_id),"stop_id":single_result["b_st"],"route_id":single_result["b_ro"],"time":{"$gte":single_result["a_r_t"]+single_result["w_t"]-18000-int((single_date - date(1970, 1, 1)).total_seconds())}}).sort( [("seq",1)] ))[0]
                b_a_t_real=real_b_seq["time"]+ int((single_date - date(1970, 1, 1)).total_seconds()) + 18000
