...
monte_carlo_range = 1800
designated_b_route_id = 31
designated_a_route_id = 1
monte_carlo_iteration = 100

print("Monte Carlo simulation range: ", monte_carlo_range, "Transfer: ", designated_a_route_id , "==>", designated_b_route_id, "running for: ", monte_carlo_iteration)

# database setup
client = MongoClient('mongodb://localhost:27017/')
db_GTFS = client.cota_gtfs
db_realtime=client.cota_real_time
db_history = client.monte_transfer
...
def convertSeconds(seconds, single_date):
    return seconds + int((single_date - date(1970, 1, 1)).total_seconds()) + \
        18000
...
def monte_carlo_transformation(t_real_time, scheduled_time):
    a = t_real_time + monte_carlo_range * rd.uniform(-1,1)
    b = scheduled_time
    # print(a, b, max(a, b))
    return max(int(a), b)
...
db_today_collection = db_history[today_date+"_"+str(monte_carlo_range)]



# data retrival
# the scheduled transfers for today
db_schedule = list(db_transfer.find({"b_service_id": str(service_id),"b_route_id":designated_b_route_id,"a_route_id":designated_a_route_id }))

db_realtime_collection=db_realtime["R"+today_date]
print(today_date, len(db_schedule))
...
for _ in range(monte_carlo_iteration):
    records_collections = []
    print("[",single_date,"]: No. "+ str(_) +" Start.")
    for each_transfer in db_schedule:
        ...
        if len(b_trip_feeds) != 0:
            b_real_time = monte_carlo_transformation(b_trip_feeds[0]["time"],  real_transfer["b_t"])
        ...
        if len(a_trip_feeds) != 0:
            a_real_time = monte_carlo_transformation(a_trip_feeds[0]["time"], real_transfer["a_t"])
        ...
        false_trips_list = list(db_seq.find({"service_id": str(
            service_id), "stop_id": b_stop_id, "route_id": real_transfer["b_ro"]})) # This is the list of trip id.
        ...
        for each_trip in false_trips_list:
            ...
            if query_realtime==[]:
                continue
            i_real_time= monte_carlo_transformation(query_realtime[0]["time"] , convertSeconds(each_trip["time"],single_date)) 
        ...
        if Total_count % 1000 ==2000:
            print("[",single_date,"]: ",Total_count,"||", Normal_count, "|", Missed_count, "|", Preemptive_count, "|", Critical_count, "|", None_count, "||",
                round((Normal_count + Missed_count + Preemptive_count + Critical_count) / Total_count,4),"||", round(Total_count/Max_count,4)  ,"||",round(b - a,2))
